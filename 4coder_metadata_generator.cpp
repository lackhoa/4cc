...
require_key_identifier(Reader *reader, String string, i64 *opt_pos_out){
    ...
    if (!success){
        ...
        String error_string = push_stringf(reader->error_arena, "expected to find '%.*s'",
...
main(int argc, char **argv){
 ...
 String cmd_out_name = push_stringf(arena, "%.*s/%s",
 ...
 String id_out_name = push_stringf(arena, "%.*s/%s",
                                    string_expand(out_directory),
                                    ID_METADATA_OUT);
