    ...
    if (chunks.node_count > 0)
    {
        ...
        for (;node != 0;)
        {
            ...
            if ((c == needle_c) ||
                (!case_sensitive && (character_to_upper(c) == character_to_upper(needle_c))))
            {
                if (c != needle_c) last_insensitive = i;
             ...
             b32 case_sensitive)
{
    ...
    switch (direction)
    {
        case Scan_Forward:
        {
            list = find_matches_forward(arena, maximum_output_count,
                                        chunks, needle, jump_table, predicate,
                                        base_index, buffer, string_id, case_sensitive);
        }break;
        
        case Scan_Backward:
        {
            list = find_matches_backward(arena, maximum_output_count,
                                         chunks, needle, jump_table, predicate,
                                         base_index, buffer, string_id, case_sensitive);
        }break;
    }
