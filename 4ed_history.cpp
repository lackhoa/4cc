...
function Record_Node*
history__to_node(Record_Node *sentinel, i1 index)
{
 Record_Node *result = 0;
 i1 counter = 0;
 Record_Node *it = sentinel;
 do{
  if (counter == index){
   result = it;
   break;
...
function void
history__merge_record_ptr_range_to_one_ptr(Record_Ptr_Lookup_Table *lookup, i1 first_one_based, i1 last_one_based,
                                           Record *record)
{
...
{
 Record_Node *sentinel = &history->free_records;
 Record_Node *new_node = sentinel->next;
...
history_get_record(History *history, i1 index){
 Record *result = 0;
 if (history->activated){
  Record_Node *node = history__to_node(history, index);
  if (node != 0){
...
history_get_sub_record(Record *record, i1 sub_index_one_based){
 ...
 if (record->kind == RecordKind_Group){
  if (0 < sub_index_one_based && sub_index_one_based <= record->group.count){
   Record_Node *node = history__to_node(&record->group.children, sub_index_one_based);
   if (node != 0){
    result = CastFromMember(Record, node, node);
   }
  }
...
function void
history__free_single_node(History *history, Record_Node *node){
 dll_remove(node);
 dll_insert(&history->free_records, node);
}

function void
history__free_nodes(History *history, i1 first_index, Record_Node *first_node, Record_Node *last_node)
{
 if (first_node == last_node){
  ...
  {
   Record_Node *left = first_node->prev;
   Record_Node *right = last_node->next;
   left->next = right;
   right->prev = left;
  }
  
  {
   Record_Node *left = &history->free_records;
   Record_Node *right = left->next;
...
history_record_edit(Global_History *global_history, History *history, Gap_Buffer *buffer,
                    i64 pos_before_edit, Edit edit, b32 automated)
{
 if (history->activated)
 {
  Assert(history->record_lookup.count == history->record_count);
  
  Record *record = history__allocate_record(history);
  history__stash_record(history, record);
  
  record->restore_point = begin_temp_memory(&history->arena);
  if (pos_before_edit >= 0) {
   record->pos_before_edit = pos_before_edit;
  } else {
   record->pos_before_edit = edit.range.min;
  }
  
  record->edit_number = global_history_get_edit_number(global_history);
  record->kind        = RecordKind_Single;
  record->automated   = automated;
  
  record->single.forward_text = push_stringz(&history->arena, edit.text);
  record->single.backward_text = buffer_stringify(&history->arena, buffer, edit.range);
  record->single.first = edit.range.first;
...
history_dump_records_after_index(History *history, i1 index){
 if (history->activated){
  ...
  if (index < history->record_count){
   Record_Node *node = history__to_node(history, index);
   Record_Node *first_node_to_clear = node->next;
   
   Record_Node *sentinel = &history->records;
   ...
   Record_Node *last_node_to_clear = sentinel->prev;
...
history_merge_records(Arena *scratch, History *history, i1 first_index, i1 last_index)
{
 if (history->activated){
  ...
  Record_Node *first_node = history__to_node(history, first_index);
  Record_Node *last_node  = history__to_node(history, last_index );
  ...
  //NOTE(kv) I don't know how to set "automated" correctly!
  new_record->automated       = first_record->automated;
  ...
  Record_Node *new_sentinel = &new_record->group.children;
  dll_init_sentinel(new_sentinel);
  
  i1 count = 0;
  for (Record_Node *node = first_node, *next = 0;
       node != 0;
       node = next)
  {
   next = node->next;
   ...
   switch (record->kind){
    ...
    {
     Record_Node *first = record->group.children.next;
     Record_Node *last  = record->group.children.prev;
