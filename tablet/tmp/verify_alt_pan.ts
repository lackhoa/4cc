// One-off check: alt+drag pans the camera and never reaches the pen handlers.
...
const ev = (x: number, y: number, altKey: boolean, buttons = 1) => ({ pointerType: "mouse", pointerId: 1, clientX: x, clientY: y, altKey, buttons, preventDefault() {} });
const before = { ...camera.pivot };
listeners.get("pointerdown")!(ev(100, 100, true));
listeners.get("pointermove")!(ev(200, 100, false)); // alt released mid-drag: still panning
