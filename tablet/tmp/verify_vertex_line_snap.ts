...
import { bezier_point, find_snap_target_stroke, nearest_point_on_stroke_world, stroke_by_id, stroke_control_points, vertex_by_id, vertex_position } from "../src/document";
...
// ids equal the old indices in this file (v3 -> v4 migration)
const host = stroke_control_points(stroke_by_id(doc, 0), doc);
...
vertex_by_id(doc, 3).position = { x: on_curve.x + 0.02, y: on_curve.y + 0.02, z: on_curve.z };
const nearest = nearest_point_on_stroke_world(stroke_by_id(doc, 0), doc, vertex_position(doc, 3));
...
console.log("vertex on curve ->", v3_length(v3_sub(vertex_position(doc, 3), bezier_point(host, pin.t))).toExponential(2), "(expect ~0)");
