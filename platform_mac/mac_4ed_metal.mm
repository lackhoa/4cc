...
metal_fill_texture(Metal_Renderer *renderer, u32 handle, Texture_Kind kind, Vec3_i32 pos, Vec3_i32 dim, void *data)
{
  ...
  if (data){
    Metal_Texture_Slot *texture_slot = [renderer get_texture_slot_at_handle:handle];
    if (texture_slot){
      Metal_Texture texture = texture_slot->texture;
      
      if (texture != 0){
        MTLRegion replace_region = {
          {(NSUInteger)pos.x, (NSUInteger)pos.y, (NSUInteger)pos.z},
...
function mac_fill_texture_sig(mac_metal__fill_texture){
    Mac_Metal *metal = (Mac_Metal*)renderer;
    b32 result = metal_fill_texture(metal->renderer, texture, texture_kind, p, dim, data);
