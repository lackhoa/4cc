...
#define M "Did not close '%.*s' because it did not successfully save."
     String str =
      push_stringfz(scratch, M, string_expand(filename));
#undef M
...
#undef M
        ...
        full_filename = push_stringfz(scratch, "%.*s/%.*s", string_expand(path), string_expand(filename));
        if (is_folder){
...

{
    for (;;){
        ...
        String full_filename = push_stringfz(scratch, "%.*s/%.*s",
