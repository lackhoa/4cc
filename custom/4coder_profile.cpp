...
profile_block__init(Thread_Context *tctx, Profile_Global_List *list,
                    String name, String location, Profile_Block *block){
    block->tctx = tctx;
    block->list = list;
    block->is_closed = false;
    block->id = thread_profile_record_push(tctx, system_time_usecond(), name, location);
}
function void
profile_block__init(Thread_Context *tctx, Profile_Global_List *list,
                    String name, String location,
...
Profile_Scope_Block::Profile_Scope_Block(App *app, String name,
                                         String location){
...
Profile_Scope_Block::close_now(){
    if (!this->is_closed){
        thread_profile_record_pop(this->tctx, system_time_usecond(), this->id);
        this->is_closed = true;
    }
}
...
Profile_Block::Profile_Block(App *app, String name,
                             String location){
    Thread_Context *v_tctx = get_thread_context(app);
    Profile_Global_List *v_list = get_core_profile_list(app);
    profile_block__init(v_tctx, v_list, name, location, this);
