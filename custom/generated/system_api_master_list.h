...
api(system) function File_Attributes quick_file_attributes(Arena* scratch, String filename);
api(system) function b32 load_handle(Arena* scratch, char* filename, Plat_Handle* out);
...
api(system) function File_Attributes save_file(Arena* scratch, char* filename, String data);
api(system) function b32 load_library(Arena* scratch, String filename, System_Library* out);
...
api(system) function u64 time_usecond(void);
...
api(system) function String get_clipboard(Arena* arena, i1 index);
api(system) function void post_clipboard(String str, i1 index);
...
api(system) function i1 thread_get_id(void);
...
api(system) function void* memory_allocate_at_least(usize wanted_size, String location, usize *usable_size_out);
api(system) function void* memory_allocate_exact(usize size, String location);
api(system) function void memory_free(void* memory0);
...
api(system) function void show_mouse_cursor(i1 show);
