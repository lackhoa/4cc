...
#define M "Node '%.*s' closed by parent ending (or higher priority sibling starting)"
                String str = push_stringf(arena, M, string_expand(name));
...
profile_draw_tab(App *app, Tab_State *state, Profile_Inspection *insp,
    ...
    v2 np = draw_fancy_string(app, state->face_id, text, fstring, state->p);
    state->p = np;
    state->p.x += state->x_half_padding;
}
...
profile_draw_node(App *app, View_ID view, Face_ID face_id,
                  ...
                  Profile_Inspection *insp, v2 m_p){
    ...
    rect2_Pair side_by_side = rect_split_left_right_lerp(Rf32(x, y), 0.5f);
    ...
    if (node->closed){
        ...
        for (Profile_Node *child = node->first_child;
            ...
            Range_f32 child_y = {};
            child_y.min = unlerp_u64(top_time.min, child_time.min, top_time.max);
            child_y.max = unlerp_u64(top_time.min, child_time.max, top_time.max);
...
profile_render(App *app, Frame_Info frame_info, View_ID view){
 ...
 if (inspect->thread_count == 0){
  ...
  v2 view_center = (region.p0 + region.p1)*0.5f;
  v2 half_dim = V2(width, line_height)*0.5f;
  v2 p = view_center - half_dim;
  draw_fancy_string(app, face_id, fcolor_zero(), fstr, p);
 }
 else{
