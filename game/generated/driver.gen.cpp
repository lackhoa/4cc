...
#define cache
#define cached
#define vref vertex_from_name
//-
global v1 head_radius_world = 9.2f * centimeter;  // @Tweak I have no idea where I pulled this number from
...
debug_frame_time_on = ReadSlider(i1, 0);
bezier_poly_nslice = ReadSlider(i1, 1);
}
...
asetf(tarm_abduct, ReadSlider(v1, 2));
if(ReadSlider(i1, 3)){
aset(tarm_bend, ReadSlider(i1, 4));
}
arest(6);
}
function void
...
{
aset(thead_phi, ReadSlider(i1, 5));
arest(1);
aset(thead_phi, ReadSlider(i1, 6));
arest(6);
blink_2_frames(shot);
}
{
aset(thead_theta, ReadSlider(i1, 7));
aset(thead_phi, ReadSlider(i1, 8));
arest(1);
aset(thead_theta, ReadSlider(i1, 9));
aset(thead_phi, ReadSlider(i1, 10));
arest(1);
aset(thead_theta, 4);
open_eyes(shot);
arest(1);
...
vertex_block("hand");
radius_scale_block(ReadSlider(v1, 11));
...
data_vv(palm_base_in, V3(-0.0476f, -2.6071f, -0.2208f));
indicate(thumb_kbot);
...
{
v3 tb_vec = V3y(ReadSlider(v1, 12));
for_i32(index,0,4){
...
data_vv(middle_palm_bot, knuckle_bots[1] + (V3(-0.f, 0.2527f, -0.022f)));
...
{
v1 finger_base_ts[FINGER_COUNT + 1] = {0.f, ReadSlider(v1, 13), ReadSlider(v1, 14), ReadSlider(v1, 15), 1.f};
v3 offset = -V3y(ReadSlider(v1, 16));
for_i32(index,0,FINGER_COUNT+1) {
finger_bases[index] = offset + bezier_sample(knuckle_line, finger_base_ts[index]);
indicate(finger_bases[index]);
}
}
v1 top_tbend = ReadSlider(v1, 17);
...
v3 xvec = V3x(ReadSlider(v1, 18));
old_vv(kmid, k + 0.5f * vec);
v3 ikmid = kmid - xvec;
v3 okmid = kmid + xvec;
{
...
v3 external_condyle;
{
data_va(internal_condyle, V3(-0.1053f, -1.0756f, -0.1775f));
data_va(external_condyle, internal_condyle + (V3(0.4031f, 0.1064f, 0.0402f)));
{
hl_block;
data_vv(sock, V3(-0.0675f, 0.733f, -0.2688f));
{
data_vv(v89, internal_condyle + V3(0.127f, 0.1897f, -0.f));
bs_parabola(sock, V3(0.137f, 0.5108f, 0.0235f), v89, lp(V4(0.5f)));
bs_unit2(v89, V4(0.f, 0.4207f, 0.f, 0.f), V3(0.5647f, -0.8253f, 0.f), internal_condyle, lp(0.5f * big_to_small()));
}
{
data_vv(a, external_condyle + V3(-0.0777f, 0.3589f, -0.0402f));
data_vv(b, sock + V3(0.2662f, -0.0344f, 0.0375f));
bs_unit2(a, V4(0.f, 1.0357f, 0.f, 0.f), V3(-0.1623f, -0.9867f, 0.f), external_condyle);
bs_parabola(b, V3(-0.0431f, 0.296f, 0.f), a, lp(.5f));
data_vv(c, b + V3(-0.152f, 0.2585f, 0.f));
bs_v3v2(b, V3(0.1369f, 0.0569f, 0.f), V2(0.0914f, 0.1488f), c, lp(.5f));
}
data_vv(v98, sock + (V3(0.0947f, 0.1683f, 0.2105f)));
data_vv(v99, v98 + (V3(-0.f, -0.6114f, -0.0063f)));
bs_parabola(v98, V3(-0.f, -0.1087f, 0.0612f), v99, lp(.9f));
data_vv(v01, v99 + (V3(0.f, -1.2272f, 0.03f)));
bs_v3v2(v99, (V3(-0.f, 0.f, -0.0834f)), V2(0.2691f, 0.0874f), v01, lp(.9f));
data_vv(v20, v01 + (V3(-0.0184f, -0.1643f, -0.0453f)));
bs_parabola(v01, ReadSlider(v3, 19), v20, lp(.9f));
}
}
data_vv(delt_bot, (V3(0.0527f, 0.0729f, -0.2028f)));
v3 delt_back;
Bez delt_cross;
Bez delt_back_line;
Bez delt_bot_line;
...
{
data_vv(delt_top, torso.shoulder + (V3(0.0328f, 0.f, 0.f)));
data_vv(delt_top_back, delt_top + (V3(-0.0109f, -0.0068f, -0.2147f)));
...
data_va(delt_back, V3(-0.0076f, 0.2268f, -0.4244f));
...
v3 white_out;
v3 tricep_mid;
data_vv(tricep_wedge, delt_bot + (V3(-0.f, -0.1879f, -0.0035f)));
data_vv(brachio_a, (V3(0.1031f, -0.7618f, -0.0498f)));
{
{
data_vv(tricep_up, delt_back_line, 0.2993f);
data_va(tricep_mid, (V3(-0.1105f, -0.1135f, -0.475f)));
data_va(white_out, tricep_mid + V3(0.0446f, -0.1341f, 0.0966f));
data_va(white_in, tricep_mid + V3(-0.1177f, -0.3581f, 0.007f));
{
...
data_vv(tricep_out_top, bezier_sample(delt_bot_line, (.3f)));
if(level1){
...
{
data_va(bicep_out_top, bezier_sample(delt_cross, ReadSlider(v1, 20)));
bicep_out_bot = ((V3(0.0079f, -0.9224f, -0.027f)) + lerp(V3(), tarm_bend, (V3())));
Bez front_out = bez_unit2(bicep_out_top, (V4(-0.0571f, 0.0667f, 0.f, 0.f)), (V3(0.7886f, 0.f, 0.6149f)), bicep_out_bot);
data_vv(hinge_out, bezier_sample(front_out, ReadSlider(v1, 21)));
data_va(bicep_in_top, bicep_out_top + (V3(-0.211f, 0.0922f, 0.0631f)));
data_va(bicep_in_bot, bicep_out_bot + (V3(-0.148f, 0.0022f, 0.0005f)));
...
data_vv(bicep_side_top, bezier_sample(delt_vfront, ReadSlider(v1, 22)));
data_vv(bicep_side_bot, bicep_out_bot + (V3(0.1103f, 0.1089f, -0.084f)));
...
data_vv(bicep_side_top2, bezier_sample(delt_cross, ReadSlider(v1, 23)));
data_vv(bicep_side_bot2, bicep_out_bot + (V3(-0.0601f, 0.f, 0.f)));
{
v1 alignment = ReadSlider(v1, 24);
...
{
b32 ok = get_view_vector().x < ReadSlider(v1, 25);
i4 radii = I4(6, 6, 0, 0);
if(get_view_vector().x > ReadSlider(v1, 26)){
radii.y = ReadSlider(i1, 27);
}
if(ok){
...
data_vv(brachio_humerus, (V3(0.0464f, -0.2894f, -0.2469f)));
{
...
data_vv(brachio_humerus2, bezier_sample(tricep_cross_line, ReadSlider(v1, 28)));
Arm arm_obj;
{
...
v3 down_common;
{
data_vv(up, palm_in + V3(-0.2986f, 1.4924f, -0.1428f));
data_vv(up1, up + (V3(0.1879f, 0.f, -0.0369f)));
data_va(down_common, palm_in + (V3(0.1191f, 0.0489f, 0.f)));
ba_parabola(l492, down_common, (V3(-0.0036f, -0.2325f, 0.f)), up1);
bs_parabola(up, (V3(0.0822f, 0.1457f, 0.f)), palm_in, params);
data_vv(v676, V3(-0.1753f, -0.854f, -0.0817f));
data_vv(v677, v676 + V3(0.f, -0.3293f, -0.0435f));
bs_v3v2(v676, V3(-0.f, 0.1052f, -0.0852f), V2(0.f, 0.0936f), v677);
}
{
data_vv(up596, l492, ReadSlider(v1, 29));
bs_parabola(down_common, (V3(0.0871f, 0.0251f, 0.f)), up596, params);
data_vv(down2, down_common + (V3(0.175f, -0.0472f, 0.f)));
data_vv(up2, up596 + V3(0.1097f, 0.161f, 0.f));
bs_parabola(down2, (V3(0.0988f, -0.2945f, 0.f)), up2, params);
}
}
data_vv(near_palm_in, palm_in + (V3(-0.0491f, 0.0154f, 0.04f)));
Bez l500;
v3 in_c;
{
data_vv(in_a, (V3(-0.2345f, -0.8464f, -0.0226f)));
data_vv(in_b, (V3(-0.269f, -1.3024f, 0.254f)));
data_va(in_c, (V3(-0.1582f, -1.8388f, 0.1688f)));
...
v3 a = arm.brachio_a;
data_va(brachio_b, a + (V3(0.1483f, -0.2286f, -0.0134f)));
data_va(brachio_c, (V3(0.2034f, -1.85f, 0.1108f)));
v3 b = brachio_b;
v3 c = brachio_c;
...
vertex_block("elbow");
v3 o = elbow_offset;
data_vv(up_in, o + (V3(-0.1512f, 0.9396f, -0.2042f)));
data_vv(low_in, o + (V3(-0.1031f, 0.7618f, -0.2006f)));
i4 elbow_radii = ReadSlider(i4, 30);
draw(bez_line(up_in, low_in), elbow_radii);
v3 up_out = elbow_up_out;
indicate(up_out, 0);
data_vv(low_out, o + (V3(-0.063f, 0.7591f, -0.2006f)));
...
data_vv(radius_bump, palm_in + V3(0.2725f, 0.1637f, -0.0384f));
{
Bez l576 = bez_v3v2(brachio_c, V3(-0.0479f, 0.0162f, 0.0042f), V2(0, 0), radius_bump);
draw(l576, lp(I4(1, 3, 3, 0)));
fill(near_palm_in, l576);
}
data_vv(v593, lerp(brachio_c, ReadSlider(v1, 31), in_c));
data_vv(radius_back, palm_in + (V3(0.2716f, 0.1061f, -0.1844f)));
{
data_vv(ulnar_ball, palm_in + V3(-0.0308f, 0.2166f, -0.0893f), 0);
data_vv(v566, (V3(-0.1204f, -1.8951f, -0.0509f)));
draw(bez_v3v2(ulnar_ball, (V3(0.0482f, 0.f, -0.f)), V2(0, 0), v566), lp(I4(0, 5, 2, 0)));
draw(bez_v3v2(arm.internal_condyle, (V3(-0.0671f, 0.f, 0.f)), V2(0, 0), v566), lp(I4(1, 5, 5, 0)));
draw_line(radius_back, radius_bump);
fill3(radius_back, v593, lerp(radius_bump, 0.1308f, near_palm_in));
}
data_vv(middle_finger_meeter, V3(0.0936f, -2.5386f, -0.171f));
...
data_vv(shoulder, V3(1.1765f, -1.6108f, 0.0573f));
send_vert(scap_sock_top);
mat4i armT = *get_bone_xform(Bone_Arm);
mat4 arm_local = ot.inv * armT;
data_vv(shoulder_in, V3(0.4819f, -1.3302f, -0.4461f));
v3 sternum = V3(0.f, -1.6696f, 0.2691f);
data_vv(delt_collar, shoulder + V3(-0.3387f, -0.02f, 0.1371f));
data_vv(sternumL, sternum + V3x(0.0919f));
{
...
data_vv(scap_delt, shoulder_in + V3(-0.1213f, -0.3863f, -0.0611f));
v3 rib_mid;
v3 rib_back;
{
v1 rib_midY = shoulder.y - 0.65f * head_unit + (-0.0775f);
data_va(rib_mid, V3(0.f, rib_midY, (0.8061f)));
{
symx_off;
draw(bez_bezd_old(sternum, V3(0.f, 0.0027f, 0.2265f), V2(0.f, 0.0935f), rib_mid));
}
data_vv(ribL, rib_mid + (V3(0.559f, -0.7158f, -0.2772f)));
...
v3 scap_sock_bot;
{
data_vv(trap_bot, V3(0.f, -3.1104f, -0.1081f));
if(level2){
hl_block;
...
draw(hip_back_line);
data_vv(back_archL, bezier_sample(hip_back_line, (0.5606f)));
...
data_vv(scap_bot, scap_delt + V3(0.1134f, -0.7434f, 0.1812f));
data_va(scap_sock_bot, scap_sock_top + V3(0.0098f, -0.2771f, 0.f));
...
bs_parabola(scap_sock_top, (V3(-0.1101f, -0.0058f, -0.0915f)), scap_sock_bot);
}
data_vv(lower_back, V3(0.f, -4.2845f, -0.0495f));
fill3(lower_back, trap_bot, back_archL);
data_vv(diamond_up, V3z(shoulder_in.z) + V3(0.f, -1.2636f, 0.0193f));
...
data_vv(diamond_low, bezier_sample(back_midline, (0.2302f)));
data_vv(diamondL, diamond_up + (V3(0.0856f, -0.2026f, -0.0719f)));
...
data_vv(trap_weird_point, bezier_sample(trap_vline, (0.5728f)));
...
data_vv(nipple, V3y(nippleY) + V3(0.5799f, 0.f, 0.6752f), 0);
data_vv(chest_in, V3(0.1045f, -2.5438f, 0.8167f));
data_vv(chest_out, V3(0.6599f, -2.4822f, 0.6002f));
data_va(pectoral_torso, chest_out + V3(0.1171f, 0.1455f, -0.1676f));
...
data_va(chest_back, (V3(0.7613f, -2.0817f, 0.039f)));
}
{
...
data_vv(navel, V3y(navelY) + V3z(0.271f), 0);
data_vv(crotch, V3());
data_vv(crotchL, V3x(0.1f));
bs_line(crotch, crotchL);
{
data_vv(bikiniL, V3(1.0685f, 0.8105f, -0.6117f));
...
v3 bikini_up_back;
{
data_vv(bikini_front_mid, V3(0.f, 0.951f, 0.1677f));
data_vv(girdle_front, bikini_front_mid + V3(0.8075f, 0.2603f, -0.0717f));
data_va(bikini_up_back, V3(0.5102f, 1.3265f, -0.7766f));
...
//  C:\Users\vodan\4ed\code\meta_klang.cpp:772:
struct Cache_Storage_34739{
b32 cache_initialized;
v1 tblink;
};
global Cache_Storage_34739 cache_storage_34739;
function void
render_eyes(Pose *pose){
v1 tblink = pose->tblink;
if(not cache_storage_34739.cache_initialized or
not(tblink == cache_storage_34739.tblink &&
true)){
{
cache_storage_34739.cache_initialized = true;
cache_storage_34739.tblink = tblink;
}
...
data_vv(eye_in, eyeT * V3(-0.0282f, 0.f, -0.1236f));
...
data_vv(eye_out, eye_in + V3(2.f * eye_in.x + (-0.03f), 0, -0.154f));
if(level1){
hl_block;
draw_line(eye_out, brow_out);
}
data_vv(es_up_in, brow_ridge, 0.3668f);
...
{
data_vv(eye_low_patch, lerp(eye_in, 0.4768f, cheek_low));
...
b32 show_loomis_ball = ReadSlider(i1, 32);
if(level2){
...
{
if(preset == 3){
show_loomis_ball = ReadSlider(i1, 33);
}
if(preset == 4){
show_loomis_ball = ReadSlider(i1, 34);
}
}
mat4i ot = *p_current_world_from_bone();
v1 noseY = -loomis_unit;
data_vv(nose_tip, V3(0.f, noseY, faceZ) + V3(0.f, 0.0237f, 0.0916f));
...
data_vv(chin_middle, V3(0, chinY, faceZ) + V3z((-0.0856f)));
v1 face_sideX = inv_root2;
v3 loomis_side_center = V3x(face_sideX);
v1 loomis_side_radius = inv_root2;
data_vv(ear_center, V3x(face_sideX) + V3(0.0416f, -0.3601f, -0.2332f));
v3 side_circle_center = V3(face_sideX, 0, 0);
v1 side_circle_radius = loomis_unit;
data_vv(chinL, V3(0, chinY, chin_middle.z) + V3(0.1572f, 0.0504f, -0.077f));
...
data_va(nose_rootL, V3(nose_sideX, lerp(browY, (0.1534f), noseY), faceZ + (-0.104f)));
data_va(nose_wing, V3(nose_wingX, nose_baseY, faceZ));
data_vv(nose_wing_up, nose_wing + V3(-0.011f, 0.1107f, 0.0069f));
v3 lip_low_center;
{
Bezier lip_up;
...
data_vv(philtrum_offset_point, nose_wing + (V3(-0.0499f, -0.0725f, -0.0489f)));
...
data_vv(v1219, addx(nose_tip, nose_sideX));
bb_unit2(nose_line_side, nose_rootL, V4(), V3(0.f, 0.f, 1.f), v1219, lp_invisible());
data_vv(nose_tipL, nose_tip + V3x(nose_sideX));
...
data_va(cheek_low, (V3(0, noseY, faceZ) + V3(0.6062f, 0.f, -0.4289f)));
fill4(jaw, chinL, cheek_low, mouth_corner);
data_va(nose_root_backL, V3(0.f, -0.0651f, -0.1755f) + V3x(nose_sideX));
data_va(brow_out, V3(0.585f, -0.1342f, 0.5485f));
data_va(cheek_up, brow_out + V3(0.f, -0.2935f, 0.f));
if(level1){
hl_block;
...
v3 brow_in = (bezier_sample(brow_ridge, 0.1313f) + V3(0.0044f, -0.0566f, 0.0056f));
indicate(brow_in);
v3 brow_mid = (bezier_sample(brow_ridge, (0.593f)) + V3(-0.0006f, 0.0008f, 0.0086f));
indicate(brow_mid);
Bezier brow_in_line = bez_line(brow_in, brow_mid);
v1 brow_joint_radius;
{
v4 radii = ReadSlider(v4, 36);
...
v4 radii = ReadSlider(v4, 37);
radii.v[0] = brow_joint_radius;
draw(brow_out_line, radii);
}
}
{
data_vv(cj, lerp(chinL, 0.5f, jaw));
{
...
data_vv(ear_back, ear_center + V3(0.1697f, 0.0034f, -0.3328f));
data_vv(ear_low, ear_center + V3(-0.0639f, -0.5264f, 0.1141f));
...
data_vv(head_back_in, V3(0.2242f, 0.6295f, -1.1258f));
...
data_vv(behind_ear, ear_center + (V3(-0.183f, -0.3342f, -0.1239f)));
...
data_vv(head_back_in2, (V3(0.4676f, -0.7579f, -0.1264f)));
...
data_vv(head_neck_junction, V3(0.f, -1.1949f, 0.3195f));
{
symx_off;
...
data_vv(a1626, V3(0.2651f, -0.5154f, 0.8715f));
data_vv(b1627, V3(0.5636f, -0.3601f, 0.5385f));
draw(bez_unit2(a1626, V4(0.f, 0.1801f, 0.0364f, 0.3141f), V3(0.866f, 0.f, 0.5f), b1627), small_to_big());
data_vv(c1633, V3(0.5266f, -0.6385f, 0.68f));
draw(bez_unit2(b1627, V4(), V3(), c1633), big_to_small());
}
b32 hair_on = ReadSlider(i1, 40);
{
...
lp_block(radii, ReadSlider(v4, 41));
v1 hairY = loomis_unit;
data_vv(hair_root, V3(0.f, 1.087f, 0.f));
data_vv(bang_root, bezier_sample(hair_hline, 0.5f));
...
{
Group(1.f, ReadSlider(v4, 42));
Group(1.f, ReadSlider(v4, 43));
Group(1.f, ReadSlider(v4, 44));
Group(0.5f, ReadSlider(v4, 45));
Group(1.f, ReadSlider(v4, 46));
}
#undef Group
...
data_vv(bang_midpoint, V3(0.f, -0.0929f, 1.042f));
{
...
data_va(bang_tip, V3(0.7418f, -0.0689f, 0.6115f) + thair * c);
}
...
Head head_obj;
{
data_vv(trapezius_head, (V3(0, noseY, head_back_out.z) + V3(0.2824f, 0.0086f, -0.323f)));
...
b32 show_body = ReadSlider(i1, 49);
...
if(camera_is_front()){
Reference_Image references[] = {{.filename = "G:/My Drive/Art/skeletal meat outline.JPG", .center = (V3(1.7822f, -6.9027f, -0.2689f)), .width = ReadSlider(v1, 50), .alpha = ReadSlider(v1, 51), .reflected = false}, {.filename = "G:/My Drive/Art/AM full body muscle front.JPG", .center = (V3(1.8527f, -6.8952f, -0.2689f)), .width = ReadSlider(v1, 52), .alpha = ReadSlider(v1, 53), .reflected = false}, {.filename = "G:/My Drive/Art/AM arm front.JPG", .center = (V3(1.2723f, -4.8425f, -0.2689f)), .width = ReadSlider(v1, 54), .alpha = ReadSlider(v1, 55), .reflected = true}, {.filename = "G:/My Drive/Art/loomis 6 heads.JPG", .center = (V3(1.0471f, -6.2154f, -0.2689f)), .width = ReadSlider(v1, 56), .alpha = ReadSlider(v1, 57), .reflected = true}, {.filename = "G:/My Drive/Art/hpc.JPG", .center = (V3(-0.5712f, -6.2775f, -0.2689f)), .width = ReadSlider(v1, 58), .alpha = ReadSlider(v1, 59)}};
i1 ref_index = preset - 4;
if(ref_index >= 0 && ref_index < alen(references)){
...
function void
compute_bones_from_pose(Modeler *m, Pose *pose){
Scratch_Block scratch;
scap_sock_top = V3(1.0676f, -1.6393f, -0.0102f);
thumb_kbot = V3(0.2669f, -2.6071f, -0.2208f);
Bone **stack = make_array(scratch, Bone * , 16);
Bone null_bone = {.xform = mat4i_identity};
array_push_value(stack, &null_bone);
#define bone_blockm(bone, xform) \
defer_block(push_bone_inner(m, stack, lr_index, bone, xform), array_pop(stack))
 
 
v1 torso_scale = head_radius_world * ReadSlider(v1, 68);
for_i32(lr_index,0,2)
 {
{
...
mat4i headT = (mat4i_scale(head_radius_world) * mat4i_rotate_tpr(head_theta, head_phi, head_roll, rotation_pivot));
if(lr_index){
headT = negateX(headT);
}
...
v1 arm_bend_max = ReadSlider(v1, 69);
...
mat4i handT = mat4i_translate(palm_in + (V3(0.0112f, 2.5863f, 0.2802f)));
bone_blockm(Bone_Hand, handT){
mat4i thumbT;
{
v1 tadduct = ReadSlider(v1, 70);
thumbT = mat4i_rotateZ(-tadduct, thumb_kbot);
}
bone_blockm(Bone_Thumb, thumbT);
v1 bot_tbend = ReadSlider(v1, 71);
i32 finger_count = 4;
knuckle_ts = {ReadSlider(v1, 72), ReadSlider(v1, 73), ReadSlider(v1, 74), ReadSlider(v1, 75)};
...
xfunction Pose
driver_animate(Modeler *m, v1 anime_time){
Pose pose;
{
...
compute_bones_from_pose(m, &pose);
return pose;
}
xfunction void
render_movie(Arena *arena, Arena *scratch, Render_Config *render_config,
             b32 references_full_alpha,
             Pose *pose, v1 anime_time){
i32 viz_level = 0;
Viewport *viewport = painter.viewport;
...
{
v3 background_hsv = ReadSlider(v3, 83);
...
if(level1){
default_fill = argb_lightness(default_fill, ReadSlider(v1, 84));
}
v1 default_line_radius_min = ReadSlider(v1, 85);
v1 default_line_end_radius = default_line_radius_min;
if(ReadSlider(i1, 86)){
default_line_end_radius = i2f6(ReadSlider(i1, 87));
}
{
Painter *p = &painter;
Modeler *m = p->modeler;
...
p->line_cparams_stack = make_array(arena, Common_Line_Params * , 32);
{
Common_Line_Params cp = get_line_cparams_list(m)[0];
cp.radii = V4(default_line_radius_min, 1.f, i2f6(ReadSlider(i1, 88)), default_line_end_radius);
argb default_line_color = argb_gray(srgb_to_linear1(ReadSlider(v1, 89)));
if(level1){
...
p->bone_stack = make_array(arena, Bone * , 16);
Bone *bones = get_bones(m);
array_push_value(p->bone_stack, &bones[0]);
push_view_vector(p, {});
}
...
{
v1 test = max(ReadSlider(v1, 90), ReadSlider(v1, 91));
v2 test2 = ReadSlider(v2, 92);
v3 test3 = ReadSlider(v3, 93);
}
}
#undef indicate
#undef X_comma_list
