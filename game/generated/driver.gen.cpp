...
//-
function Pose
driver_animate(Modeler *m, v1 anim_time);

global Arena driver_dll_arena;

function Pose
driver_update(Modeler *m, v1 anim_time){
Pose result = driver_animate(m, anim_time);
return result;
}
function void
driver_reload(Framework_API *framework_api){
#define X(N) N = framework_api->N;
...
asetf(tarm_abduct, ReadSlider(v1, 0));
if(ReadSlider(i1, 1)){
aset(tarm_bend, ReadSlider(i1, 2));
}
arest(6);
}
function void
...
{
aset(thead_phi, ReadSlider(i1, 3));
arest(1);
aset(thead_phi, ReadSlider(i1, 4));
arest(6);
blink_2_frames(shot);
}
{
aset(thead_theta, ReadSlider(i1, 5));
...
v1 torso_scale = head_radius_world * ReadSlider(v1, 9);
...
v1 arm_bend_max = ReadSlider(v1, 10);
...
mat4i thumbT;
{
v1 tadduct = ReadSlider(v1, 11);
thumbT = mat4i_rotateZ(-tadduct, thumb_kbot);
}
bone_blockm(Bone_Thumb, thumbT);
v1 bot_tbend = ReadSlider(v1, 12);
i32 finger_count = 4;
knuckle_ts = {ReadSlider(v1, 13), ReadSlider(v1, 14), ReadSlider(v1, 15), ReadSlider(v1, 16)};
...
v3 finger_vecs_[4] = {-V3y(ReadSlider(v1, 17)), -V3y(ReadSlider(v1, 18)), -V3y(ReadSlider(v1, 19)), -V3y(ReadSlider(v1, 20))};
...
vertex_block("hand");
radius_scale_block(ReadSlider(v1, 24));
...
old_vv(palm_base_in, V3(-0.0476f, -2.6071f, -0.2208f));
indicate(thumb_kbot);
...
{
v3 tb_vec = V3y(ReadSlider(v1, 25));
for_i32(index,0,4){
...
old_vv(middle_palm_bot, knuckle_bots[1] + (V3(-0.f, 0.2527f, -0.022f)));
...
{
v1 finger_base_ts[FINGER_COUNT + 1] = {0.f, ReadSlider(v1, 26), ReadSlider(v1, 27), ReadSlider(v1, 28), 1.f};
v3 offset = -V3y(ReadSlider(v1, 29));
...
v1 top_tbend = ReadSlider(v1, 30);
for_i32(ifinger,0,FINGER_COUNT){
bone_block(make_bone_id(Bone_Bottom_Phalanx, ifinger));
v3 k = knuckle_tops[ifinger];
v3 vec = finger_vecs[ifinger];
v3 xvec = V3x(ReadSlider(v1, 31));
...
v3 external_condyle;
{
old_va(internal_condyle, V3(-0.1053f, -1.0756f, -0.1775f));
old_va(external_condyle, internal_condyle + V3(0.4031f, 0.1064f, 0.0402f));
{
hl_block;
old_vv(sock, V3(-0.0675f, 0.733f, -0.2688f));
{
old_vv(v89, internal_condyle + V3(0.127f, 0.1897f, -0.f));
...
{
old_vv(a, external_condyle + V3(-0.0777f, 0.3589f, -0.0402f));
old_vv(b, sock + V3(0.2662f, -0.0344f, 0.0375f));
bs_unit2(a, V4(0.f, 1.0357f, 0.f, 0.f), V3(-0.1623f, -0.9867f, 0.f), external_condyle);
bs_parabola(b, V3(-0.0431f, 0.296f, 0.f), a, lp(.5f));
old_vv(c, b + V3(-0.152f, 0.2585f, 0.f));
bs_v3v2(b, V3(0.1369f, 0.0569f, 0.f), V2(0.0914f, 0.1488f), c, lp(.5f));
}
old_vv(v98, sock + (V3(0.0947f, 0.1683f, 0.2105f)));
old_vv(v99, v98 + (V3(-0.f, -0.6114f, -0.0063f)));
bs_parabola(v98, V3(-0.f, -0.1087f, 0.0612f), v99, lp(.9f));
old_vv(v01, v99 + (V3(0.f, -1.2272f, 0.03f)));
bs_v3v2(v99, (V3(-0.f, 0.f, -0.0834f)), V2(0.2691f, 0.0874f), v01, lp(.9f));
old_vv(v20, v01 + (V3(-0.0184f, -0.1643f, -0.0453f)));
bs_parabola(v01, ReadSlider(v3, 32), v20, lp(.9f));
}
}
old_vv(delt_bot, (V3(0.0527f, 0.0729f, -0.2028f)));
v3 delt_back;
Bez delt_cross;
Bez delt_back_line;
Bez delt_bot_line;
...
{
old_vv(delt_top, torso.shoulder + (V3(0.0328f, 0.f, 0.f)));
old_vv(delt_top_back, delt_top + (V3(-0.0109f, -0.0068f, -0.2147f)));
...
old_va(delt_back, V3(-0.0076f, 0.2268f, -0.4244f));
...
v3 white_out;
v3 tricep_mid;
old_vv(tricep_wedge, delt_bot + (V3(-0.f, -0.1879f, -0.0035f)));
old_vv(brachio_a, (V3(0.1031f, -0.7618f, -0.0498f)));
{
{
old_vv(tricep_up, bezier_sample(delt_back_line, 0.2993f));
old_va(tricep_mid, (V3(-0.1105f, -0.1135f, -0.475f)));
old_va(white_out, tricep_mid + V3(0.0446f, -0.1341f, 0.0966f));
old_va(white_in, tricep_mid + V3(-0.1177f, -0.3581f, 0.007f));
{
...
old_vv(tricep_out_top, bezier_sample(delt_bot_line, (.3f)));
if(level1){
...
{
old_va(bicep_out_top, bezier_sample(delt_cross, ReadSlider(v1, 33)));
...
old_vv(hinge_out, bezier_sample(front_out, ReadSlider(v1, 34)));
old_va(bicep_in_top, bicep_out_top + (V3(-0.211f, 0.0922f, 0.0631f)));
old_va(bicep_in_bot, bicep_out_bot + (V3(-0.148f, 0.0022f, 0.0005f)));
...
old_vv(bicep_side_top, bezier_sample(delt_vfront, ReadSlider(v1, 35)));
old_vv(bicep_side_bot, bicep_out_bot + (V3(0.1103f, 0.1089f, -0.084f)));
...
old_vv(bicep_side_top2, bezier_sample(delt_cross, ReadSlider(v1, 36)));
old_vv(bicep_side_bot2, bicep_out_bot + (V3(-0.0601f, 0.f, 0.f)));
{
v1 alignment = ReadSlider(v1, 37);
...
{
b32 ok = get_view_vector().x < ReadSlider(v1, 38);
i4 radii = I4(6, 6, 0, 0);
if(get_view_vector().x > ReadSlider(v1, 39)){
radii.y = ReadSlider(i1, 40);
}
if(ok){
...
old_vv(brachio_humerus, (V3(0.0464f, -0.2894f, -0.2469f)));
{
...
old_vv(brachio_humerus2, bezier_sample(tricep_cross_line, ReadSlider(v1, 41)));
Arm arm_obj;
{
...
v3 down_common;
{
old_vv(up, palm_in + V3(-0.2986f, 1.4924f, -0.1428f));
old_vv(up1, up + (V3(0.1879f, 0.f, -0.0369f)));
old_va(down_common, palm_in + (V3(0.1191f, 0.0489f, 0.f)));
ba_parabola(l492, down_common, (V3(-0.0036f, -0.2325f, 0.f)), up1);
bs_parabola(up, (V3(0.0822f, 0.1457f, 0.f)), palm_in, params);
old_vv(v676, V3(-0.1753f, -0.854f, -0.0817f));
old_vv(v677, v676 + V3(0.f, -0.3293f, -0.0435f));
bs_v3v2(v676, V3(-0.f, 0.1052f, -0.0852f), V2(0.f, 0.0936f), v677);
}
{
old_vv(up596, bezier_sample(l492, ReadSlider(v1, 42)));
bs_parabola(down_common, (V3(0.0871f, 0.0251f, 0.f)), up596, params);
old_vv(down2, down_common + (V3(0.175f, -0.0472f, 0.f)));
old_vv(up2, up596 + V3(0.1097f, 0.161f, 0.f));
bs_parabola(down2, (V3(0.0988f, -0.2945f, 0.f)), up2, params);
}
}
old_vv(near_palm_in, palm_in + (V3(-0.0491f, 0.0154f, 0.04f)));
Bez l500;
v3 in_c;
{
old_vv(in_a, (V3(-0.2345f, -0.8464f, -0.0226f)));
old_vv(in_b, (V3(-0.269f, -1.3024f, 0.254f)));
old_va(in_c, (V3(-0.1582f, -1.8388f, 0.1688f)));
...
v3 a = arm.brachio_a;
old_va(brachio_b, a + (V3(0.1483f, -0.2286f, -0.0134f)));
old_va(brachio_c, (V3(0.2034f, -1.85f, 0.1108f)));
v3 b = brachio_b;
v3 c = brachio_c;
...
vertex_block("elbow");
v3 o = elbow_offset;
old_vv(up_in, o + (V3(-0.1512f, 0.9396f, -0.2042f)));
old_vv(low_in, o + (V3(-0.1031f, 0.7618f, -0.2006f)));
i4 elbow_radii = ReadSlider(i4, 43);
draw(bez_line(up_in, low_in), elbow_radii);
v3 up_out = elbow_up_out;
indicate(up_out, 0);
old_vv(low_out, o + (V3(-0.063f, 0.7591f, -0.2006f)));
...
old_vv(radius_bump, palm_in + V3(0.2725f, 0.1637f, -0.0384f));
{
Bez l576 = bez_v3v2(brachio_c, V3(-0.0479f, 0.0162f, 0.0042f), V2(0, 0), radius_bump);
draw(l576, lp(I4(1, 3, 3, 0)));
fill(near_palm_in, l576);
}
old_vv(v593, lerp(brachio_c, ReadSlider(v1, 44), in_c));
old_vv(radius_back, palm_in + (V3(0.2716f, 0.1061f, -0.1844f)));
{
old_vv(ulnar_ball, palm_in + V3(-0.0308f, 0.2166f, -0.0893f), 0);
old_vv(v566, (V3(-0.1204f, -1.8951f, -0.0509f)));
...
old_vv(middle_finger_meeter, V3(0.0936f, -2.5386f, -0.171f));
...
draw(l618);
old_vv(v612, bezier_sample(l618, 0.7612f));
...
old_vv(shoulder, V3(1.1765f, -1.6108f, 0.0573f));
send_vert(scap_sock_top);
mat4i armT = *get_bone_xform(Bone_Arm);
mat4 arm_local = ot.inv * armT;
old_vv(shoulder_in, V3(0.4819f, -1.3302f, -0.4461f));
v3 sternum = V3(0.f, -1.6696f, 0.2691f);
old_vv(delt_collar, shoulder + V3(-0.3387f, -0.02f, 0.1371f));
old_vv(sternumL, sternum + V3x(0.0919f));
{
...
old_vv(scap_delt, shoulder_in + V3(-0.1213f, -0.3863f, -0.0611f));
v3 rib_mid;
v3 rib_back;
{
v1 rib_midY = shoulder.y - 0.65f * head_unit + (-0.0775f);
old_va(rib_mid, V3(0.f, rib_midY, (0.8061f)));
{
symx_off;
draw(bez_bezd_old(sternum, V3(0.f, 0.0027f, 0.2265f), V2(0.f, 0.0935f), rib_mid));
}
old_vv(ribL, rib_mid + (V3(0.559f, -0.7158f, -0.2772f)));
...
v3 scap_sock_bot;
{
old_vv(trap_bot, V3(0.f, -3.1104f, -0.1081f));
if(level2){
hl_block;
...
draw(hip_back_line);
old_vv(back_archL, bezier_sample(hip_back_line, (0.5606f)));
...
old_vv(scap_bot, scap_delt + V3(0.1134f, -0.7434f, 0.1812f));
old_va(scap_sock_bot, scap_sock_top + V3(0.0098f, -0.2771f, 0.f));
...
old_vv(lower_back, V3(0.f, -4.2845f, -0.0495f));
fill3(lower_back, trap_bot, back_archL);
old_vv(diamond_up, V3z(shoulder_in.z) + V3(0.f, -1.2636f, 0.0193f));
...
old_vv(diamond_low, bezier_sample(back_midline, (0.2302f)));
old_vv(diamondL, diamond_up + (V3(0.0856f, -0.2026f, -0.0719f)));
...
old_vv(trap_weird_point, bezier_sample(trap_vline, (0.5728f)));
...
old_vv(nipple, V3y(nippleY) + V3(0.5799f, 0.f, 0.6752f), 0);
old_vv(chest_in, V3(0.1045f, -2.5438f, 0.8167f));
old_vv(chest_out, V3(0.6599f, -2.4822f, 0.6002f));
old_va(pectoral_torso, chest_out + V3(0.1171f, 0.1455f, -0.1676f));
...
old_va(chest_back, (V3(0.7613f, -2.0817f, 0.039f)));
}
{
...
old_vv(navel, V3y(navelY) + V3z(0.271f), 0);
old_vv(crotch, V3());
old_vv(crotchL, V3x(0.1f));
bs_line(crotch, crotchL);
{
old_vv(bikiniL, V3(1.0685f, 0.8105f, -0.6117f));
...
v3 bikini_up_back;
{
old_vv(bikini_front_mid, V3(0.f, 0.951f, 0.1677f));
old_vv(girdle_front, bikini_front_mid + V3(0.8075f, 0.2603f, -0.0717f));
old_va(bikini_up_back, V3(0.5102f, 1.3265f, -0.7766f));
...
v3 eye_scale = V3(0.8324f, 0.9882f, 0.92f);
mat4i eyeT = mat4i_translate(eyeO) * mat4i_scales(eye_scale);
old_vv(eye_in, eyeT * V3(-0.0282f, 0.f, -0.1236f));
...
old_vv(eye_out, eye_in + V3(2.f * eye_in.x + (-0.03f), 0, -0.154f));
if(level1){
hl_block;
draw_line(eye_out, brow_out);
}
old_vv(es_up_in, bezier_sample(brow_ridge, 0.3668f));
...
{
old_vv(eye_low_patch, lerp(eye_in, 0.4768f, cheek_low));
...
b32 show_loomis_ball = ReadSlider(i1, 45);
if(level2){
...
{
if(preset == 3){
show_loomis_ball = ReadSlider(i1, 46);
}
if(preset == 4){
show_loomis_ball = ReadSlider(i1, 47);
}
}
mat4i ot = *p_current_world_from_bone();
v1 noseY = -loomis_unit;
old_vv(nose_tip, V3(0.f, noseY, faceZ) + V3(0.f, 0.0237f, 0.0916f));
...
old_vv(chin_middle, V3(0, chinY, faceZ) + V3z((-0.0856f)));
v1 face_sideX = inv_root2;
v3 loomis_side_center = V3x(face_sideX);
v1 loomis_side_radius = inv_root2;
old_vv(ear_center, V3x(face_sideX) + V3(0.0416f, -0.3601f, -0.2332f));
v3 side_circle_center = V3(face_sideX, 0, 0);
v1 side_circle_radius = loomis_unit;
old_vv(chinL, V3(0, chinY, chin_middle.z) + V3(0.1572f, 0.0504f, -0.077f));
...
old_va(nose_rootL, V3(nose_sideX, lerp(browY, (0.1534f), noseY), faceZ + (-0.104f)));
old_va(nose_wing, V3(nose_wingX, nose_baseY, faceZ));
old_vv(nose_wing_up, nose_wing + V3(-0.011f, 0.1107f, 0.0069f));
v3 lip_low_center;
{
Bezier lip_up;
...
old_vv(philtrum_offset_point, nose_wing + (V3(-0.0499f, -0.0725f, -0.0489f)));
...
old_vv(v1219, addx(nose_tip, nose_sideX));
bb_unit2(nose_line_side, nose_rootL, V4(), V3(0.f, 0.f, 1.f), v1219, lp_invisible());
old_vv(nose_tipL, nose_tip + V3x(nose_sideX));
...
old_va(cheek_low, (V3(0, noseY, faceZ) + V3(0.6062f, 0.f, -0.4289f)));
fill4(jaw, chinL, cheek_low, mouth_corner);
old_va(nose_root_backL, V3(0.f, -0.0651f, -0.1755f) + V3x(nose_sideX));
old_va(brow_out, V3(0.585f, -0.1342f, 0.5485f));
old_va(cheek_up, brow_out + V3(0.f, -0.2935f, 0.f));
if(level1){
hl_block;
...
v4 radii = ReadSlider(v4, 50);
radii.v[0] = brow_joint_radius;
draw(brow_out_line, radii);
}
}
{
old_vv(cj, lerp(chinL, 0.5f, jaw));
{
...
old_vv(ear_back, ear_center + V3(0.1697f, 0.0034f, -0.3328f));
old_vv(ear_low, ear_center + V3(-0.0639f, -0.5264f, 0.1141f));
...
old_vv(head_back_in, V3(0.2242f, 0.6295f, -1.1258f));
...
old_vv(behind_ear, ear_center + (V3(-0.183f, -0.3342f, -0.1239f)));
...
old_vv(head_back_in2, (V3(0.4676f, -0.7579f, -0.1264f)));
...
old_vv(head_neck_junction, V3(0.f, -1.1949f, 0.3195f));
{
symx_off;
...
old_vv(a1626, V3(0.2651f, -0.5154f, 0.8715f));
old_vv(b1627, V3(0.5636f, -0.3601f, 0.5385f));
draw(bez_unit2(a1626, V4(0.f, 0.1801f, 0.0364f, 0.3141f), V3(0.866f, 0.f, 0.5f), b1627), small_to_big());
old_vv(c1633, V3(0.5266f, -0.6385f, 0.68f));
draw(bez_unit2(b1627, V4(), V3(), c1633), big_to_small());
}
b32 hair_on = ReadSlider(i1, 53);
{
...
lp_block(radii, ReadSlider(v4, 54));
v1 hairY = loomis_unit;
old_vv(hair_root, V3(0.f, 1.087f, 0.f));
old_vv(bang_root, bezier_sample(hair_hline, 0.5f));
...
{
Group(1.f, ReadSlider(v4, 55));
Group(1.f, ReadSlider(v4, 56));
Group(1.f, ReadSlider(v4, 57));
Group(0.5f, ReadSlider(v4, 58));
Group(1.f, ReadSlider(v4, 59));
}
#undef Group
...
old_vv(bang_midpoint, V3(0.f, -0.0929f, 1.042f));
{
...
old_va(bang_tip, V3(0.7418f, -0.0689f, 0.6115f) + thair * c);
}
...
Head head_obj;
{
old_vv(trapezius_head, (V3(0, noseY, head_back_out.z) + V3(0.2824f, 0.0086f, -0.323f)));
...
b32 show_body = ReadSlider(i1, 62);
v1 arm_ry = head_unit_world * (0.5302f);
...
if(camera_is_front()){
Reference_Image references[] = {{.filename = "G:/My Drive/Art/skeletal meat outline.JPG", .center = (V3(1.7822f, -6.9027f, -0.2689f)), .width = ReadSlider(v1, 63), .alpha = ReadSlider(v1, 64), .reflected = false}, {.filename = "G:/My Drive/Art/AM full body muscle front.JPG", .center = (V3(1.8527f, -6.8952f, -0.2689f)), .width = ReadSlider(v1, 65), .alpha = ReadSlider(v1, 66), .reflected = false}, {.filename = "G:/My Drive/Art/AM arm front.JPG", .center = (V3(1.2723f, -4.8425f, -0.2689f)), .width = ReadSlider(v1, 67), .alpha = ReadSlider(v1, 68), .reflected = true}, {.filename = "G:/My Drive/Art/loomis 6 heads.JPG", .center = (V3(1.0471f, -6.2154f, -0.2689f)), .width = ReadSlider(v1, 69), .alpha = ReadSlider(v1, 70), .reflected = true}, {.filename = "G:/My Drive/Art/hpc.JPG", .center = (V3(-0.5712f, -6.2775f, -0.2689f)), .width = ReadSlider(v1, 71), .alpha = ReadSlider(v1, 72)}};
...
function void
render_movie(Arena *arena, Painter *painter0, Pose *pose, v1 anime_time){
painter = painter0;
i32 viz_level = 0;
...
if(level1){
default_fill = argb_lightness(default_fill, ReadSlider(v1, 81));
}
v1 default_line_radius_min = ReadSlider(v1, 82);
v1 default_line_end_radius = default_line_radius_min;
if(ReadSlider(i1, 83)){
default_line_end_radius = i2f6(ReadSlider(i1, 84));
}
{
Painter *p = painter;
Modeler *m = p->modeler;
...
cp.radii = V4(default_line_radius_min, 1.f, i2f6(ReadSlider(i1, 85)), default_line_end_radius);
argb default_line_color = argb_gray(srgb_to_linear1(ReadSlider(v1, 86)));
if(level1){
...
render_reference_images();
{
v1 test = max(ReadSlider(v1, 88), ReadSlider(v1, 89));
v2 test2 = ReadSlider(v2, 90);
v3 test3 = ReadSlider(v3, 91);
}
painter = 0;
}
dll_export void
driver_api_export(Driver_API *api){
api->is_valid = true;
#define X(N) api->N = N;
