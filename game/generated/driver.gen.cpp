//NOTE Generated by C:\Users\vodan\4ed\code/meta_klang.cpp:574:
...
bezier_poly_nslice = fval_text(16);}
...
asetf(tarm_abduct, fval_text(0.0211f, f20th));
if(fbool(0)){
aset(tarm_bend, fval_text(11));
}
arest(6);}
function void
...
{
aset(thead_theta, fval_text(-8));
aset(thead_phi, fval_text(-3));
arest(1);
aset(thead_theta, fval_text(-7));
aset(thead_phi, fval_text(0));
arest(1);
aset(thead_theta, 4);
open_eyes(shot);
arest(1);
...
vertex_block("hand");
radius_scale_block(fval_text(0.5038f));
...
{
data_va(function_condyle, V3(-0.1053f, -1.0756f, -0.1775f));
data_va(external_condyle, function_condyle+(V3(0.4031f, 0.1064f, 0.0402f)));
{
hl_block;
data_vv(sock, V3(-0.0675f, 0.733f, -0.2688f));
{
data_vv(v89, function_condyle+V3(0.127f, 0.1897f, -0.f));
...
data_vv(v20, v01+fvec(V3(-0.0184f, -0.1643f, -0.0453f)));
bs_parabola(v01, fval_text(V3(0.f, 0.0368f, 0.1927f)), v20, lp(.9f));
}
}
...
{
data_va(bicep_out_top, bezier_sample(delt_cross, fval_text(0.8389f)));
...
data_vv(hinge_out, bezier_sample(front_out, fval_text(0.6758f)));
...
data_vv(bicep_side_top, bezier_sample(delt_vfront, fval_text(0.9539f)));
...
data_vv(bicep_side_top2, bezier_sample(delt_cross, fval_text(0.7406f)));
data_vv(bicep_side_bot2, bicep_out_bot+(V3(-0.0601f, 0.f, 0.f)));
{
v1 alignment = fval_text(0.9169f);
...
{
b32 ok = get_view_vector().x < fval_text(0.07f);
i4 radii = I4(6, 6, 0, 0);
if(get_view_vector().x > fval_text(-0.20f)){
radii.y = fvali(0);
}
if(ok){
...
data_vv(brachio_humerus2, bezier_sample(tricep_cross_line, fval_text(0.4296f)));
Arm arm_obj;
{
...
{
data_vv(up596, l492, fval_text(0.9006f));
...
i4 elbow_radii = fval_text(I4(4, 3, 3, 2));
...
data_vv(v593, lerp(brachio_c, fval_text(0.1619f), in_c));
...
return pelvis_obj;}
//  C:\Users\vodan\4ed\code/meta_klang.cpp:495:
struct Cache_Storage_35066{
b32 cache_initialized;
v1 tblink;
};
global Cache_Storage_35066 cache_storage_35066;
function void
render_eyes(Pose *pose){

v1 tblink = pose->tblink;
if(not cache_storage_35066.cache_initialized or
not(tblink == cache_storage_35066.tblink &&
true)){
{
cache_storage_35066.cache_initialized = true;
cache_storage_35066.tblink = tblink;
}
...
data_va(cheek_low, (V3(0,noseY,faceZ) +
...
Head head_obj;
{
data_vv(trapezius_head, (V3(0, noseY, head_back_out.z) + 
...
Reference_Image references[] =
   {
    ...
    },
    {
     ...
     .width   = fval_text(2.6661f),
     .alpha   = fval_text(0.4221f),
    },
   };
...
v1 width = fval_text(2.4645f);
char *filename = "G:/My Drive/Art/left arm back.PNG";
draw_image(filename, center, V3x(-width), V3y(1.f), fval_text(0.3033f));
}else if(preset == 4){
...
v1 torso_scale = head_radius_world*fval_text(1.2484f);
for_i32(lr_index,0,2)
 {
{
...
const v1 arm_bend_max = fval_text(-0.5f);
...
if(level1){
default_fill = argb_lightness(default_fill, fval_text(0.7797f));
}
v1 default_line_radius_min = fval_text(0.5089f);
v1 default_line_end_radius = default_line_radius_min;
if(fbool(1)){
default_line_end_radius = i2f6(fval_text(2));
}
{
Painter &p = painter;
Modeler &m = *p.modeler;
...
argb default_line_color = argb_gray(srgb_to_linear1(fval_text(0.3389f)));
if(level1){
...
arrayof<Bone> &bones = get_bones(m);
...
{
v1 test = *(v1 *)read_slider_at_index(0);
v2 test2 = *(v2 *)read_slider_at_index(1);
DEBUG_VALUE(test);
DEBUG_VALUE(test2);
}}
#undef indicate
