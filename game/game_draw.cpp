...
{
 v3 points[] = {a,b,c};
 poly3_inner_2(points, color, depth_offset, flags);
}
//-TODO(kv) Ferry these functions over to the framework
function void
             ...
             linum_defparam)
{
 if(is_poly_enabled()){
  ...
  poly3_inner_2(points, params.color, depth_offset, flags);
 }
}
inline void
...
compute_fill_color(v1 color_lerp){
 argb color = painter->line_params.fill.color;
 if(color_lerp != 0.0f){
