...
set_draw_location(Location location)
{
 ...
 // NOTE(kv) The cursor-derived hot_location is empty when the cursor sits outside any
 // marked range -- so an invalid location must never count as hot. (Document draws
 // use the document_location variant, see replay_recording.)
...
// plane (or, with Ctrl, define a new one and swing the other handle into it --
// plan-handle-drag-modes), a vertex move rotates both offsets with the chord.
// rotate_about_axis is unused since roll went away (2026-09-15), kept for the next caller.
