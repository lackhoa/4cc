...
def_set_config_u64(App *app, String_ID key, u64 val){
    Scratch_Block scratch(app);
    String val_string = push_stringf(scratch, "%llu", val);
    ...
    for (File_Info **ptr = list.infos, **end = list.infos + list.count;
        ...
        if (!HasFlag(info->attributes.flags, FileAttribute_IsDirectory)){
            ...
            if (string_match(string_postfix(name, 7), str8_lit(".4coder"))){
                ...
                String full_name = push_stringf(scratch, "%.*s/%.*s",
                                                            string_expand(path),
                                                            string_expand(name));
...
load_theme_current_buffer(App_Cmd *app)
{
    ...
    if (filename.size > 0){
        ...
        if (error_text.size > 0 || problem_score >= 10){
            String string = push_stringf(scratch, "There appears to be a problem parsing %.*s; no theme change applied\n", string_expand(filename));
