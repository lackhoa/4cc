...
function void 
select_surrounding_scope(App_Cmd *app)
{
...
function void 
select_surrounding_scope_maximal(App_Cmd *app)
{
    View_ID view = get_active_view(app, Access_ReadVisible);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
    i64 pos = view_get_cursor_pos(app, view);
...
function void 
select_next_scope_absolute(App_Cmd *app)
{
 View_ID view = get_active_view(app, Access_ReadVisible);
 Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
 i64 pos = view_get_cursor_pos(app, view);
...
function void 
select_next_scope_after_current(App_Cmd *app)
{
    View_ID view = get_active_view(app, Access_ReadVisible);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
...
function void 
select_prev_scope_absolute(App_Cmd *app)
{
    View_ID view = get_active_view(app, Access_ReadVisible);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadVisible);
    i64 pos = view_get_cursor_pos(app, view);
...
function void 
select_prev_top_most_scope(App_Cmd *app)
{
...
function void 
place_in_scope(App_Cmd *app)
{
 place_begin_and_end_on_own_lines(app, "{", "}");
}

function void 
delete_current_scope(App_Cmd *app)
{
