...
function API_Definition*
api_get_api(API_Definition_List *list, String name){
...
function API_Call*
api_get_call(API_Definition *api, String name){
    API_Call *result = 0;
...
function API_Type*
api_get_type(API_Definition *api, String name){
    API_Type *result = 0;
...
generate_header(API_Definition *api, Stringz source_name, API_Generation_Flag flags, FILE *out)
{
 ...
 String api_prefix = {};
 b32 has_prefix = flags & APIGeneration_PrefixCallables;
 if(has_prefix){
  api_prefix = push_stringf(scratch, "%.*s_", strexpand(api->name));
 }
 ...
 for (API_Call *call = api->first_call;
      call != 0;
      call = call->next)
 {//-the weird signature wrapper (not used)
  fprintf(out, "#define %.*s_%.*s_sig() %.*s %.*s_%.*s(",
          strexpand(api->name),
          strexpand(call->name),
          strexpand(call->return_type),
          strexpand(api->name),
          strexpand(call->name));
  if (call->params.count == 0){
   fprintf(out, "void");
   ...
   for (API_Param *param = call->params.first;
        ...
        param = param->next){
    fprintf(out, "%.*s %.*s",
            strexpand(param->type_name),
            strexpand(param->name));
    if (param->next != 0){
     fprintf(out, ", ");
    }
   }
 ...
 for (API_Call *call = api->first_call;
      ...
      call = call->next)
 {
  fprintf(out, "#define %.*s%.*s__return %.*s\n",
          strexpand(api_prefix),
          strexpand(call->name),
          strexpand(call->return_type));
  {//-Params
   fprintf(out, "#define %.*s%.*s__params ",
           strexpand(api_prefix), strexpand(call->name));
   if(call->params.count == 0){
    fprintf(out, "void");
   }else{
    for(API_Param *param = call->params.first;
 ...
 {//-vtable struct
  fprintf(out, "#if defined(STATIC_LINK_API) || defined(DYNAMIC_LINK_API)\n");
  fprintf(out, "struct API_VTable_%.*s{\n", strexpand(api->name));
  for (API_Call *call = api->first_call;
       call != 0;
       call = call->next)
  {
   fprintf(out, "wrap_function_pointer(%.*s%.*s);\n",
           strexpand(api_prefix), strexpand(call->name));
  }
  fprintf(out, "};\n");
  fprintf(out, "#endif\n");
 ...
 fprintf(out, "#elif defined(DYNAMIC_LINK_API)\n");
 {
  ...
  {
   String callable_name = api_get_callable_name(scratch, api->name, call->name, flags);
   fprintf(out, "STORAGE_CLASS wrap_function_pointer(%.*s);\n",
           strexpand(callable_name));
  }
 }
                                     ...
                                     API_Generation_Flag flags)
{
 ...
 
 {
  ...
  for_i32(index,0,filecount)
  {
   ...
   it.name = pjoin(scratch, meta.dirs.code,
 ...
 for_i32(index,0,filecount)
 {
  ...
  if (it.out)
  {
   myfprintf(it.out, "// Auto-generated by command %s\n\n", to_cstring(meta_command_name));
  }
  else
  {
...
function void
api_definition_error(Arena *arena, List_String *list,
...
function void
api_definition_check(Arena *arena, API_Definition *correct, API_Definition *remote, API_Check_Flag flags, List_String *error_list){
...
function void
api_list_check(Arena *arena, API_Definition_List *correct, API_Definition_List *remote, API_Check_Flag flags, List_String *error_list){
    b32 report_missing = HasFlag(flags, APICheck_ReportMissingAPI);
    b32 report_extra = HasFlag(flags, APICheck_ReportExtraAPI);
