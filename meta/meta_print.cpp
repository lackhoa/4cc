...
{//@the_type_wrapper_debacle
 ...
 {//-The struct
  print_format(p, strcode(struct %S\n), wrapper);
  PrintBraces(p)
  {
   print(p,"\n");
   
   if(is_v3){ print(p, strlit("union{\n")); }
   print_format(p, strcode(%S v;\n), wrapped);
   if(is_v3)
   ...
   print_format(p, strcode(myinline operator %S(){ return v; }\n), wrapped);
  }
  print(p, ";\n");
 }
 ...
 if(do_info)
 {
  ...
  m_braces_newline
  {
   ...
   p<"result.name = "<enclosed_in_strlit(type_name) < ";\n";
   p<"result.size = sizeof("<type_name<");\n";
   print_format(p, strcode(result.kind = I_Type_Kind_Wrapper;\n));
   print_format(p, strcode(result.constructor = strcode(%S);\n), constructor);
   print_format(p, strcode(result.wrapped_type = type_info_of(%S);\n), wrapped);
   p < "return result;";
  }
...
{// NOTE see meta_klang.cpp
 ...
 {//-The infos
  m_locationp(printer);
  {// NOTE The array
   print_format(printer, "global Slider global_sliders[%d];", sliders.count);
   print(printer, "\n\n");
  }
 }
 ...
 {//-The meat
  ...
  print(printer, "{\n");
  {
   for_i32(slider_index, 0, sliders.count)
   {
    ...
    // bookmark so the hack is that Basic_Type_Count is
    // the type id for @FUI_Line_Params
    String type = push_stringf(tmp, "types.%S", slider.type);
    // NOTE @mk_slider_
    print_format(printer, "global_sliders[%d] = mk_slider_(%S, %d, %d, %d, %d, %S);\n",
                 slider_index,
                 type, file, range.min, range.max, slider_index, options);
...
print_all_script_data(Driver_Collected &driver)
{
 ...
 Stringz h_path   = pjoin(tmp, meta.dirs.game, strlit("sliders0.gen.h"));
 Stringz cpp_path = pjoin(tmp, meta.dirs.game, strlit("sliders0.gen.cpp"));
