...
function i64
view_pos_from_xy(App *app, View_ID view, v2 p)
{
...
view_move_buffer_point(App *app, View_ID view, Buffer_Point buffer_point, v2 delta){
...
view_set_cursor_and_preferred_x(App *app, View_ID view, Buffer_Seek seek){
 ...
 v2 p = view_relative_xy_of_pos(app, view, cursor.line, cursor.pos);
...
query_user_number(App_Cmd *app, Query_Bar *bar, i32 x){
    Scratch_Block scratch(app);
    String string = push_stringf(scratch, "%d", x);
...
place_begin_and_end_on_own_lines(App_Cmd *app, char *begin, char *end){
    ...
    if ((lines.min < lines.max) || (!min_line_blank)){
        ...
        String str = push_stringf(scratch, "%s\n\n%s", begin, end);
    ...
    if (top.line > 0){
        ...
        if (bottom.line > 0){
            ...
            if (target_height + 2*line_height > view_height){
                ...
                Buffer_Scroll scroll = view_get_buffer_scroll(app, view);
                v2 top_p = view_relative_xy_of_pos(app, view, scroll.position.line_number, range.min);
                ...
                if (top_p.y < acceptable_y.min){
                    ...
                    v2 bot_p = view_relative_xy_of_pos(app, view, scroll.position.line_number, range.max);
...
search_up_path(Arena *arena, String start_path, String filename){
 ...
 for (;path.size > 0;)
 {
  ...
  Stringz full_path = push_stringf(arena, "%.*s/%.*s",
                                    string_expand(path),
