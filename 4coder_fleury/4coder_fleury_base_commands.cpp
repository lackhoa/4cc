...
_F4_PushListerOptionForNote(App *app, Arena *arena, Lister *lister, F4_Index_Note *note)
{
    if(note && note->file)
    {
        ...
        String name = push_stringf(arena, "[%.*s] %.*s", string_expand(buffer_name), string_expand(note->string));
        String sort = S8Lit("");
        switch(note->kind)
        {
...
F4_ReIndentLine(App *app, Buffer_ID buffer, i64 line, i64 indent_delta)
{
    ...
    // NOTE(rjf): Indent lines.
    {
        ...
        String indent_string = indent_with_tabs ? S8Lit("\t") : push_stringf(scratch, "%.*s", Min(indent_width, 16),
                                                                                      "                ");
...
CUSTOM_DOC("Counts the lines of code in the current buffer, breaks it down by section, and outputs to the *loc* buffer.")
{
    ...
    if(loc_buffer != 0)
    {
        ...
        for(int i = 0; i < info_count; i += 1)
        {
            ...
            String string = push_stringf(scratch2,
