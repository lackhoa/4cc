...
function void
vim_draw_visual_mode(App *app, View_ID view, Buffer_ID buffer, Face_ID face_id, Text_Layout_ID text_layout_id){
 ...
 {//-Build status
  if(not dirty)
  {
   ...
   if(state.valid){
	...
	draw_string(app, face_id, push_stringf(scratch, "%d,%d", cursor.line, cursor.col), draw_p, base_color);
	...
	if(cursor_position == 0){
		...
		PosText = push_stringf(scratch, "%d%%", i64(100.L*cast(f64)cursor_position/(f64)(buffer_size)));
	}
...
function Rect_f32
vim_draw_query_bars(App *app, Rect_f32 region, View_ID view_id, Face_ID face_id)
{
	...
	if(get_active_query_bars(app, view_id, ArrayCount(space), &query_bars)){
		foreach(i,query_bars.count){
			rect2_Pair pair = layout_query_bar_on_bot(region, line_height, 1);
...
function rect2_Pair
vim_line_number_margin(App *app, Buffer_ID buffer, Rect_f32 rect, f32 digit_advance)
{
    ...
    rect2_Pair pair = rect_split_left_right(rect, margin_width);
...
function void
vim_draw_rel_line_number_margin(App *app, View_ID view, Buffer_ID buffer, Face_ID face, Text_Layout_ID text_layout_id, Rect_f32 margin){
...
function void
vim_draw_line_number_margin(App *app, View_ID view, Buffer_ID buffer, Face_ID face, Text_Layout_ID text_layout_id, Rect_f32 margin){
...
vim_draw_whole_screen(App *app, Frame_Info frame_info)
{
 ...
 Face_ID face_id = get_face_id(app, 0);
 Face_Metrics metrics = get_face_metrics(app, face_id);
 v1 line_height = metrics.line_height;
 v1 char_wid = metrics.normal_advance;
 
 // NOTE(BYP): Drawing the cursor for view transitions
 if(vim_cur_cursor_pos != vim_nxt_cursor_pos)
 {
  v2 cursor_dim = V2(9.f, (vim_state.mode == VIM_Insert ? 4.f : 18.f));
  rect2 cursor_rect = Rf32_xy_wh(vim_cur_cursor_pos - cursor_dim, cursor_dim);
  u64 cursor_roundness_100 = def_get_config_u64(app, vars_intern_lit("cursor_roundness"));
  v1 roundness = char_wid*(v1)cursor_roundness_100*0.01f;
 ...
 rect2 back_rect = vim_get_bottom_rect(app);
 {// NOTE(BYP): Drawing the back of the filebar lister
  if(vim_cur_lister_offset > vim_nxt_lister_offset)
  {
 ...
 {// NOTE: Draw bottom text
  ...
  {// NOTE(kv): Normal bottom text
   String bottom_text = vim_get_bottom_text();
   draw_string(app, face_id, bottom_text, main_monitor_bot_left, finalize_color(defcolor_text_default, 0));
  }
 }
 ...
 {//note(kv) I have no idea what this is, not the bottom lister
  ...
  v1 ratio_diff = entry->nxt_ratio - entry->cur_ratio;
  ...
  buffer_point.pixel_shift.y = line_height*entry->cur_ratio*(v1)(line_count+1) - rect_height(back_rect);
  
  FColor peek_back_color = fcolor_id(defcolor_back);
  draw_rect_fcolor(app, back_rect, 0.f, peek_back_color);
  rect2_Pair pair = rect_split_top_bottom_neg(back_rect, 4.f);
 ...
 if(vim_state.chord_resolved)
 {
  chord_color = (vim_state.chord_resolved & bit_2 ?
                 ...
                 fcolor_id(defcolor_vim_chord_text));
 }
